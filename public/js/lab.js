...
let vectors = [
    ...
    new Vector2D(w + 200, 800),
];

let distance = new Vector2D(Math.abs(vectors[1].x - vectors[0].x), Math.abs(vectors[1].y - vectors[0].y));

ctx.beginPath();
ctx.strokeStyle = "#ffffff";
ctx.moveTo(vectors[0].x, vectors[0].y);
ctx.lineTo(vectors[1].x, vectors[1].y);
ctx.closePath();
ctx.stroke();
...
let [arc_x, arc_y] = [
    x, 
    vectors[0].y + ((compute_y(distance, x - vectors[0].x)) * ((vectors[0].y > vectors[1].y) ? -1 : 1)),
];
ctx.arc(arc_x, arc_y, 10, 0, Math.PI * 2);
ctx.closePath();
ctx.fill();

console.log(compute_y(new Vector2D(Math.abs(vectors[1].x - vectors[0].x), Math.abs(vectors[1].y - vectors[0].y)), x));
