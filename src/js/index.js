...
const mouse_down = (e) => {
    ...
    if (e.buttons == 1) {
        ...
        const { x, y, z } = new_block;
        if (!scene.camera.is_collide([new_block], scene.camera.position.x, scene.camera.position.y, scene.camera.position.z)) {
            scene.add_block(x, y, z, colors[selected_color]);
        }
    } else if (e.buttons == 2) {
...
for (let i = 2; i < 36; i++) {
    for (let j = 2; j < 36; j++) scene.add_block(i * 50, 0, j * 50);
}
